...
import PackagesBooking from "../models/Packages/packagesbookingModel.js";
...
export const getAllAirport = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch airports", error: error.message });
  }
};
...
export const getFlightsByFilter = async (req, res) => {
  ...
  if (!from || !to || !departureDate) {
    return res.status(400).json({
      success: false,
      message: "Fields 'from', 'to', and 'departureDate' are required.",
    });
  }
  ...
  try {
    ...
    const allFlights = await Flight.find({
      operatingDays: { $in: [departureDay] },
    }).populate("fromAirport toAirport");
    ...
    const findPaths = (path, currentAirportId, visited) => {
      if (toAirportIdsSet.has(currentAirportId)) {
        ...
        if (allLegsHaveSeats) results.push([...path]);
        return;
      }
...
export const getFlightById = async (req, res) => {
  ...
  if (!flightIds) {
    return res.status(400).json({ message: 'flightIds query parameter is required' });
  }

  const flightIdArray = flightIds
    .split(',')
    .map(id => id.trim())
    .filter(id => mongoose.Types.ObjectId.isValid(id));

  if (flightIdArray.length === 0) {
    return res.status(400).json({ message: 'flightIds must contain valid ObjectIds' });
  }
  ...
  try {
    ...
    for (const flightId of flightIdArray) {
      ...
      // -------- PackageBookings (Package Based Flight) --------
      ...
      const packageBookings = await UnifiedBooking.find({
        ...
        status: { $in: 'confirmed'},
        $or: [
          // Match traveler with same flight and same date
          {
            'details.travelers': {
              $elemMatch: {
          ...
          // Match transport with same flight and same date
          {
            'details.transports': {
              $elemMatch: {
                mode: 'Flight',
          ...
          // Match flight entry in flights array
          {
            'details.flights': {
              $elemMatch: {
                flight: new mongoose.Types.ObjectId(flightId),
...
export const flightBooking = async (req, res) => {
  ...
  if (!travelers || !Array.isArray(travelers) || travelers.length === 0) {
    return res.status(400).json({ message: 'Travelers info is required' });
  }
  if (!email || !email.trim()) {
    return res.status(400).json({ message: 'Contact email is required' });
  }
  if (!flights || !Array.isArray(flights) || flights.length === 0) {
    return res.status(400).json({ message: 'Flights info is required' });
  }
  ...
  try {
    ...
    for (const flightSegment of flights) {
      ...
      // Find existing bookings from unified Booking model
